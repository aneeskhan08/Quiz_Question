...
switch (questions.q2) {
  case "A. tsc":
    console.log(chalk.italic.bold.greenBright("2. Correct !"));
    ++score;
    break;
  default:
...
switch (questions.q3) {
  case "B. To enforce Static Typing and catch error at compile time":
    console.log(chalk.italic.bold.greenBright("3. Correct !"));
    ++score;
    break;
  default:
...
switch (questions.q4) {
  case "B. export":
    console.log(chalk.italic.bold.greenBright("4. Correct !"));
    ++score;
    break;
  default:
...
switch (questions.q5) {
  case "B. To create interactive command-line interfaces ":
    console.log(chalk.italic.bold.greenBright("5. Correct !"));
    ++score;
    break;
  default:
...
switch (questions.q6) {
  case "A. import {greet} from './module2';":
    console.log(chalk.italic.bold.greenBright("6. Correct !"));
    ++score;
    break;
  default:
...
switch (questions.q7) {
  case "B. A text based Data format used for storing and transferring data":
    console.log(chalk.italic.bold.greenBright("7. Correct !"));
    ++score;
    break;
  default:
...
switch (questions.q8) {
  case "B. {'key':'value'}":
    console.log(chalk.italic.bold.greenBright("8. Correct !"));
    ++score;
    break;
  default:
...
switch (questions.q9) {
  case "A. explicit":
    console.log(chalk.italic.bold.greenBright("9. Correct !"));
    ++score;
    break;
  default:
...
switch (questions.q10) {
  case "B. string and number":
    console.log(chalk.italic.bold.greenBright("10. Correct !"));
    ++score;
    break;
  default:
...
console.log(
  chalk.italic.blue(
    `\n\t <<<<---- ${chalk.bold.magenta(
      questions.myName
    )}: Score ${chalk.bold.magenta(score)} out of ${chalk.bold.magenta(
      10
    )} ---->>>> 👍`
  )
);